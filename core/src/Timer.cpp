...
Timer::Timer() : m_dRateToSeconds(0.0), m_uTickFrequency(0), m_uStartClock(0), m_fElapsedSeconds(0.0f) {
	uint64_t rate;
...
void Timer::BeginTimer() {
	m_uStartClock = Timer::GetTicks();
}

void Timer::EndTimer() {
	const uint64_t endClock = Timer::GetTicks();
